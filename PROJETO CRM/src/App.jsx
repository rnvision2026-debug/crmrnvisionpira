...
function NegotiationsPage({ profile, profiles, services, leads, activities, settings, reload, notify }) {
  ...
  const [adminEditingStatus, setAdminEditingStatus] = useState(false)
  const [adminStatusDraft, setAdminStatusDraft] = useState('novo')
  ...
  useEffect(() => {
    setAdminEditingStatus(false)
    setAdminStatusDraft(lead?.status || 'novo')
  }, [lead?.id, lead?.status])
  ...
  return <div className="grid gap negotiations-page clean-negotiations">
    ...
    {profile.role === 'admin' && <section className={`card template-card ${templateOpen ? 'open' : 'collapsed'}`}>
      <div className="form-head template-head">
        <div><h3>Mensagem automática do WhatsApp</h3><p className="muted">Configure o texto que abre no WhatsApp do vendedor. O campo fica fechado para não poluir a tela.</p></div>
        <button type="button" className={templateOpen ? 'ghost' : 'primary'} onClick={() => setTemplateOpen(v => !v)}>{templateOpen ? 'Fechar edição' : 'Editar mensagem'}</button>
      </div>
      {templateOpen && <>
        ...
        <div className="message-preview"><small>Prévia com o lead selecionado</small><p>{lead ? renderTemplate(templateDraft, { vendedor: profile.role === 'admin' ? (seller?.name || profile.name) : profile.name, cliente: lead.client_name || 'Cliente', empresa: lead.company_name || 'Empresa', servico: service?.name || 'Site profissional', valor: money(lead.proposal_value || initialValue(service)) }) : renderTemplate(templateDraft, { vendedor: profile.name, cliente: 'Cliente', empresa: 'Empresa', servico: 'Site profissional', valor: money(1599) })}</p></div>
      </>}
    </section>}
          ...
          <div className="primary-actions-grid">
            {profile.role === 'admin' ? (
              <div className="status-control">
                {!adminEditingStatus ? <>
                  <div className="status-readonly"><small>Status atual</small><strong>{statusLabel(lead.status)}</strong></div>
                  <button type="button" className="ghost" onClick={() => { setAdminStatusDraft(lead.status || 'novo'); setAdminEditingStatus(true) }}>Editar status</button>
                </> : <>
                  <select value={adminStatusDraft} onChange={e => setAdminStatusDraft(e.target.value)}>{STATUS.map(([id, label]) => <option key={id} value={id}>{label}</option>)}</select>
                  <button type="button" className="primary" onClick={() => changeStatus(adminStatusDraft)}>Salvar status</button>
                  <button type="button" onClick={() => { setAdminStatusDraft(lead.status || 'novo'); setAdminEditingStatus(false) }}>Cancelar</button>
                </>}
              </div>
            ) : (
